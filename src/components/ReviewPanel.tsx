...
const ReviewPanel = ({ review }: Props) => {
  ...
  const handleShare = () => {
    const bugsSection = review.bugs.length > 0
      ? `### 🐛 Bugs Found (${review.bugs.length})\n${review.bugs.map(b => `- **Line ${b.line}:** ${b.issue} → _Fix: ${b.fix}_`).join("\n")}\n`
      : "";
    const securitySection = review.security_issues.length > 0
      ? `### 🔒 Security Issues\n${review.security_issues.map(s => `- ${s}`).join("\n")}\n`
      : "";
    const text = [
      `## CodeReview AI — Review Summary`,
      ``,
      `**Score:** ${review.quality_score}/10 | **Time:** ${review.time_complexity.split(".")[0]} | **Space:** ${review.space_complexity.split(".")[0]}`,
      ``,
      `> ${review.summary}`,
      ``,
      bugsSection,
      ...
      ``,
      `---`,
      `_Generated by [CodeReview AI](https://codereview.ai)_`,
    ].filter(Boolean).join("\n");
    navigator.clipboard.writeText(text);
    toast.success("PR-ready review summary copied to clipboard!");
  };
